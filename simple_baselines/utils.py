...
def lstm(xs,ms,s,scope,nh,init_scale=1.0):
    ...
    for idx,(x,m) in enumerate(zip(xs,ms)):
        ...
        i,f,o,u=tf.split(value=z,num_or_size_splits=4,axis=1)
        i=tf.nn.sigmoid(i)
        f=tf.nn.sigmoid(f)
        o=tf.nn.sigmoid(o)
        u=tf.tanh(u)
...
def batch_to_seq(h,nbatch,nsteps,flat=False):
    ...
    return [tf.squeeze(v,[1]) for v in tf.split(value=h,num_or_size_splits=nsteps,axis=1)]

def seq_to_batch(h,flat=False):
    if not flat:
        nh=h[0].get_shape()[-1].value
        return tf.reshape(tf.concat(values=h,axis=1),[-1,nh])
    else:
